...
namespace Nop.Web.Components.Forms
{
    ...
    /// </summary>
    public class NopEditForm : ComponentBase
    {
        ...
        /// <summary>
        /// Constructs an instance of <see cref="NopEditForm"/>.
        /// </summary>
        public NopEditForm()
        {
        ...
        /// <summary>
        /// Specifies the content to be rendered inside this <see cref="NopEditForm"/>.
        /// </summary>
        ...
        protected override void OnParametersSet()
        {
            ...
            {
                throw new InvalidOperationException($"{nameof(NopEditForm)} requires a {nameof(Model)} " +
                    $"parameter, or an {nameof(EditContext)} parameter, but not both.");
            }
            else if (!_hasSetEditContextExplicitly && Model == null)
            {
                throw new InvalidOperationException($"{nameof(NopEditForm)} requires either a {nameof(Model)} " +
            ...
            if (OnSubmit.HasDelegate && (OnValidSubmit.HasDelegate || OnInvalidSubmit.HasDelegate))
            {
                throw new InvalidOperationException($"When supplying an {nameof(OnSubmit)} parameter to " +
                    $"{nameof(NopEditForm)}, do not also supply {nameof(OnValidSubmit)} or {nameof(OnInvalidSubmit)}.");
            }
